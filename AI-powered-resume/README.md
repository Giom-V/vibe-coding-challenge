...
* For each line, save (bug hidden), on top of what's already written a longer version of what I did (I will review it myself, but you can write down a first version). And add a gemini button that can be pressed to ask Gemini more about what I exactly did during this job (of course, don't forget to prompt the model to sell me under my best profil, not to criticize me). There could be some prefilled prompts provided.
* Add a button to generate an audio overview of my profile (maybe with preset so one can ask to focus on one aspect on my professional life). See below how to use the TTS model to generate the audio overview 
...
Finding a good balace between when I want to harcode or precisely steer, and what I want to be dynamic
